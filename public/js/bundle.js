...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"../../node_modules/qr-scanner/src/qr-scanner.js":[function(require,module,exports) {
"use strict";
...
class QrScanner {
  ...
  static hasCamera() {
    ...
    return navigator.mediaDevices.enumerateDevices().then(devices => devices.some(device => device.kind === 'videoinput')).catch(() => false);
  }

  constructor(video, onDecode, canvasSizeOrOnDecodeError = this._onDecodeError.bind(this), canvasSize = QrScanner.DEFAULT_CANVAS_SIZE, preferredFacingMode = 'environment') {
    this.$video = video;
  ...
  hasFlash() {
    ...
    return imageCapture.getPhotoCapabilities().then(result => {
  ...
  static createQrEngine(workerPath = QrScanner.WORKER_PATH) {
    return ('BarcodeDetector' in window ? BarcodeDetector.getSupportedFormats() : Promise.resolve([])).then(supportedFormats => supportedFormats.indexOf('qr_code') !== -1 ? new BarcodeDetector({
      formats: ['qr_code']
  ...
  _scanFrame() {
    ...
    requestAnimationFrame(() => {
      ...
      this._qrEnginePromise.then(qrEngine => QrScanner.scanImage(this.$video, this._sourceRect, qrEngine, this.$canvas, true)).then(this._onDecode, error => {
...
var qrScanner = new _qrScanner.default(videoElem, function (result) {
  return console.log("decoded qr code:", result);
});

_qrScanner.default.hasCamera().then(function () {
...
qrScanner.start();
},{"qr-scanner":"../../node_modules/qr-scanner/src/qr-scanner.js"}],"../../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
